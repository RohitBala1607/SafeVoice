  ...
  navigator.geolocation.getCurrentPosition(async (position) => {
    ...
    try {
      ...
      const sosResponse = await api.post('/sos/start', {
        ...
        location: { lat, lng },
        mapsLink,
        contacts: config.whatsappShare ? contacts : []
      });
      ...
      // 📲 WhatsApp Alert (Frontend Tab Fallback)
      if (config.whatsappShare && contacts.length > 0) {
        console.log("📲 Opening WhatsApp alerts in browser tabs...");
...
const startAudioRecording = async () => {
  try {
    ...
    console.log("⏺️ Recording started (5s limit)...");

    // 🕒 Auto-stop after 5 seconds
    setTimeout(() => {
      if (mediaRecorder && mediaRecorder.state !== "inactive") {
        console.log("⏱️ 5s limit reached. Stopping recorder...");
